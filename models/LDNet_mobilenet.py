...
def model_base():
    ...
    base=list(base.children())[:-1]
    _base=nn.Sequential(*base)
    return _base


class LDNet_mobile(nn.Module):
    def __init__(self,n_classes=5,img_ch=1,output_ch=1):
        super(LDNet_mobile,self).__init__()
        ...
        self.upsample=up_conv_vgg(ch_in=1280,ch_out=64)
